...
class ImageClassifier:
    ...
    def train(
        ...
        time.sleep(0.1)  # Prevents a race condition between tqdm and print statements.
        print("\nTraining SVM...")
        self._reverse_labels = {v: k for k, v in self._labels.items()}
    ...
    def predict_image(self, image):
        ...
        descriptors = self._dictionary.compute(
            gray, self._bow.feature_extractor.detect(image)
        )
        ...
        predicted_labels = [int(label) for label in predicted_labels][0]

        predicted_label_name = f"{self._reverse_labels[predicted_labels]}"
