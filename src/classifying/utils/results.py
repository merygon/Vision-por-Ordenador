...
class Results:
    ...
    def compute(
        self, dataset: List[str], true_labels: List[int], predicted_labels: List[int]
        ...
        for image, true, predicted in zip(dataset, true_labels, predicted_labels):
            classification.append(
                (os.path.basename(image), descriptions[true], descriptions[predicted])
            )
    ...
    def print(self, accuracy: float, confusion_matrix: np.ndarray):
        ...
        labels = [
            key for key, value in sorted(self._labels.items(), key=lambda x: x[1])
        ]
    ...
    def save(
        self, confusion_matrix: np.ndarray, classification: List[Tuple[str, str, str]]
    ):
        ...
        classification_df = pd.DataFrame(
            classification, columns=("Image", "Known", "Predicted")
        )

        # Write to Excel
        workbook = (
            self._dataset_name.lower().replace(" ", "_") + "_"
            if self._dataset_name
            else ""
        )
        ...
        with pd.ExcelWriter(workbook) as writer:
            confusion_df.to_excel(
                writer, sheet_name="Confusion matrix", index_label="KNOWN/PREDICTED"
            )
